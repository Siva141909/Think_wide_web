...
// seen records every event_id processed so far, across the whole file (not
// just the current batch), so a duplicate arriving in a later batch is
// still caught. It is only touched from processBatch's sequential
// dedup loop, which runs on the main goroutine before workers start.
var seen = map[string]bool{}

// openedBy remembers which (campaign, contact) pairs have already opened,
// so repeat opens by the same contact do not inflate unique_opens — but a
// contact opening in a different campaign still counts there. It is only
// touched from track(), which runs on the main goroutine.
